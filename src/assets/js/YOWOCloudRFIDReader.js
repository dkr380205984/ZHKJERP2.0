...
let YOWORFIDReader = {
  createNew: function () {
    ...
    // let bExitFromWait = false
    let SplitChar = String.fromCharCode(65530)
    let Timer
    let CList
    let STimer
    let WSonOpen = function () {
      SocketOpen = true
      yw.Ver()
    }
    let WSonMessage = function (evt) {
      let t = evt.data.split(SplitChar)
      let resultData = {
        type: 'Result',
    ...
    yw.TryConnect = function () {
      try {
        ...
        if (Timer == null) { Timer = setInterval(TickOn, 20000) }
        if (CList == null) { CList = new ArrayList() }
        if (STimer == null) { STimer = setInterval(st, 100) }
        return true
      } catch (ex) {
        return false
      }
    ...
    let s = function (FuncName, FunctionID, ParamStr) {
      let PrixStr
    ...
    yw.EY5K_ReadUserFile = function (SamIndex, Addr, Count, Format) {
      s('EY5K_ReadUserFile', 60, SamIndex + SplitChar + Addr + SplitChar + Count + SplitChar + Format)
    }

    yw.EY5K_WriteUserFile = function (SamIndex, Addr, DataStr, Format) {
      s('EY5K_WriteUserFile', 61, SamIndex + SplitChar + Addr + SplitChar + DataStr + SplitChar + Format)
    }
    ...
    yw.MultiInventory15693Start = function () {
      s('MultiInventory15693Start', 70, '')
    }

    yw.MultiInventory15693Stop = function () {
      s('MultiInventory15693Stop', 71, '')
    }
