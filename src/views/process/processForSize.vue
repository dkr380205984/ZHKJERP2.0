                      ...
                      <div class="trow"
                        ...
                        <div class="tcolumn">
                          <el-checkbox @change="$forceUpdate()"
                            v-model="itemChild.checked">{{itemChild.size_name}}</el-checkbox>
                        </div>
                        <div class="tcolumn"
                          style="color:#01B48C"
                        ...
                        <div class="tcolumn">
                          <span class="btn noBorder"
                            ...
                            @click="singleAllocation(item.product_id,itemChild.size_id,'',itemChild.numbers)">分配</span>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div class="normalTb">
                ...
                <div class="tbody">
                  <div class="trow"
                    ...
                    <div class="tcolumn"
                      style="flex:1.8">{{item.size_name}}</div>
                    <div class="tcolumn"
                    ...
                    <div class="tcolumn noPad"
                      ...
                      <div class="trow"
                        ...
                        <div class="tcolumn">
                          <span class="btn noBorder"
                            style="padding:0;margin:0"
                            @click="singleStockIn(item.product_id,itemChild.size_id,'')">入库</span>
                        </div>
                      </div>
                    </div>
                  </div>
  ...
  methods: {
          ...
          item.companyRate.forEach((itemClient) => {
            item.mixedData.forEach((itemColor) => {
              let finded = list.find((itemFind) => {
                return itemFind.product_id === item.product_id && itemFind.client_id === itemClient.company_id && itemFind.process === itemClient.process && itemFind.size_id === itemColor.colorSize
              })
              if (finded) {
                ...
                list.push({
                  ...
                  color_name: item.colorSizeArr.find((itemFind) => Number(itemFind.size_id) === Number(itemColor.colorSize)).size_name
                })
              }
            })
          })
    ...
    getInspectionPro (ev) {
      ...
      this.renderData.allocationDetailCommon.find((item) => item.product_id === ev).childrenMergeInfo.forEach((item) => {
        this.commonFind(colorSizeArr, item, ['size_id'], [])
      })
      ...
      this.renderData.allocationDetailCommon.forEach((item) => {
        item.childrenMergeInfo.forEach((itemChild) => {
          ...
          resArr[0].data.data.filter((item) => item.production_type === this.otherData.processType).forEach((itemLog) => {
            if (item.product_id === itemLog.product_id && itemChild.size_id === itemLog.size_id) {
    ...
    getAllocation (resArr) {
      ...
      if (this.otherData.processType === '织片') {
        ...
        this.renderData.orderInfo.batch_info.forEach((itemBatch) => {
          itemBatch.product_info.forEach((itemPro) => {
            this.commonFind(allocationList, itemPro, ['size_id', 'product_id'], ['numbers'])
          })
        })
        ...
        this.$clone(resArr[0].data.data).filter((item) => item.process === '织片').forEach((item) => {
          this.commonFind(allocationList, item, ['product_id', 'size_id'], ['number', 'motorise_number'])
        })
        this.renderData.allocationList = this.$mergeData(allocationList.map((item) => {
      ...
      commonLog.forEach((item) => {
        ...
        item.style_name = item.product_info.style_name
        item.product_code = item.product_info.product_code
        this.commonFind(allocationDetailCommon, item, ['product_id', 'size_id'], ['number', 'motorise_number'])
      })
