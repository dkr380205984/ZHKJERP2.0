      ...
      <div class="listCtn">
        ...
        <div class="filterCtn2"
          ...
          <div class="btn btnBlue"
            @click="$router.push('/receiveDispatch/SJBB/page=1&&order_code=&&client_name=&&date=')">数据报表</div>
        </div>
...
export default {
  ...
  methods: {
    ...
    changeRouter (page) {
      let pages = page || 1
      this.$router.push('/receiveDispatch/receiveDispatchList/page=' + pages + '&&keyword=' + this.$changeSpecialWord(this.keyword, true) + '&&date=' + this.date + '&&group_id=' + this.group_id + '&&company_id=' + this.company_id + '&&state=' + this.state + '&&searchOrderOrProduct=' + this.searchOrderOrProduct)
    },
    reset () {
