        ...
        <div class="rowCtn">
          <div class="colCtn"
            ...
            <div class="flexTb">
              ...
              <div class="tbody">
                <div class="trow"
                  v-for="itemClient in renderData.newAllocation"
                  :key="itemClient.process">
                  <div class="tcolumn">
                    <el-checkbox v-model="itemClient.checked"
                      style="align-items: center;display: flex;"
                      @change="checkAllColorSize($event,itemClient)">
                  ...
                  <div class="tcolumn noPad"
                    ...
                    <div class="trow"
                      ...
                      <div class="tcolumn">
                        <span style="color:#01B48C">{{itemChild.product_info.product_code}}</span>
                      ...
                      <div class="tcolumn">{{itemChild.checkNum}}</div>
                      <div class="tcolumn"
                        style="color:#F5222D">{{itemChild.cpNum}}</div>
                      <div class="tcolumn">
                        <div class="opr"
                          @click="addLog(itemChild.product_id, itemChild.size_id, itemChild.color_id,itemChild.numbers,itemClient.process,itemChild)">新增记录</div>
                      </div>
                    </div>
                  </div>
                </div>
...
export default {
  ...
  methods: {
    ...
    getCommon (resArr) {
      ...
      // 注意这部分冗余代码是因为产前信息确认代码不是我写的，但是依旧要用到这部分数据，所以保留历史数据，新增renderData.newAllocation字段用于新的业务逻辑
      ...
      let newAllocationMerge = this.$mergeData(allocationAdd, {
        mainRule: ['process']
      })
      this.renderData.newAllocation = newAllocationMerge
      ...
      this.selectData.processArr = [{
        value: '已分配工序',
        label: '已分配工序',
        children: newAllocationMerge.map((item) => {
          return {
            value: item.process,
            label: item.process
          }
        ...
        value: '所有工序',
        label: '所有工序',
        children: resArr[2].data.data.map((item) => {
          return {
            value: item.name,
            label: item.name
          }
    ...
    getReceiveDispatch (resArr) {
      ...
      this.renderData.statistics.forEach((itemPro) => {
        itemPro.childrenMergeInfo.forEach((itemChild) => {
          const findInfo = this.nativeData.log.filter((itemFind) => itemFind.product_id === itemPro.product_id && itemFind.size_id === itemChild.size_id && itemFind.color_id === itemChild.color_id)
          itemChild.childrenMergeInfo = this.$mergeData(findInfo, {
            mainRule: 'process',
            otherRule: [
    ...
    batchAddLog () {
      ...
      this.renderData.newAllocation.forEach((itemPro) => {
        itemPro.childrenMergeInfo.forEach((itemChild) => {
