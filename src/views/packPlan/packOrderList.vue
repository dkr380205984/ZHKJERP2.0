...
export default {
  ...
  methods: {
    ...
    changeRouter (page) {
      let pages = page || 1
      this.$router.push('/packPlan/packOrderList/page=' + pages + '&&keyword=' + this.keyword + '&&date=' + this.date + '&&group_id=' + this.group_id + '&&company_id=' + this.company_id + '&&state=' + this.state)
    },
    reset () {
      this.$router.push('/packPlan/packOrderList/page=1&&keyword=&&date=&&group_id=&&company_id=&&state=')
    },
    getOrderList () {
      this.loading = true
      order.list({
