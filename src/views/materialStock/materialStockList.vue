...
export default {
  ...
  methods: {
    ...
    changeRouter (page) {
      let pages = page || 1
      this.$router.push('/materialStock/materialStockList/page=' + pages + '&&keyword=' + this.keyword + '&&date=' + this.date + '&&group_id=' + this.group_id + '&&company_id=' + this.company_id + '/' + (this.orderType ? '1' : '2'))
    },
    reset () {
      this.$router.push('/materialStock/materialStockList/page=1&&keyword=&&date==&&group_id=&&company_id=/1')
    },
    getOrderList () {
      this.loading = true
