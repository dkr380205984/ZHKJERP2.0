...
export default {
  data () {
    return {
      ...
      // warnType: '',
      // warnList: [],
      // timeData: [{ percent: 0.1, name: '物料计划' }, { percent: 0.25, name: '物料入库' }, { percent: 0.35, name: '半成品入库' }, { percent: 0.15, name: '成品入库' }, { percent: 0.15, name: '成品装箱' }],
      // orderTypeArr: []
    }
  },
  methods: {
  ...
  created () {
    ...
    Promise.all([
      ...
      this.orderTypeArr = res[2].data.data.map(item => {
        return {
          value: item.name
        }
      })
