...
<script>
import { productProcessSJBB, client } from '@/assets/js/api.js'
...
export default {
  ...
  methods: {
    ...
    changeRouter (page) {
      let pages = page || 1
      this.$router.push('/productProcess/SJBB/page=' + pages + '&&order_code=' + this.order_code + '&&client_id=' + this.client_id + '&&date=' + this.date)
    },
    reset () {
      this.$router.push('/productProcess/SJBB/page=1&&order_code=&&client_id=&&date=')
    },
    getFilters () {
    ...
    getList () {
      ...
      productProcessSJBB.list({
        ...
        this.list.forEach((item) => {
          item.shoddy_number = item.rejects_info ? JSON.parse(item.rejects_info).number : 0
        })
        this.list = this.$mergeData(this.list, {
    ...
    getHistory (id) {
      this.loading = true
      productProcessSJBB.history({
        id
      }).then((res) => {
        console.log(res)
        this.historyList = res.data.data.sort((a, b) => {
          return (new Date(a.complete_time).getTime() - new Date(b.complete_time).getTime())
        })
        let shoddyTotal = 0
        let total = 0
