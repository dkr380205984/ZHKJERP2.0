<template>
  <div id="productList"
    ...
    <div class="listCutCtn">
      <div class="cut_item"
        @click="$router.push('/order/orderList/page=1&&keyword=&&date=&&has_materialOrder=&&has_materialPlan=&&has_materialStock=&&has_weave=&&has_productInOut=&&has_inspection=&&has_boxing=&&group_id=&&company_id=&&state=&&searchOrderOrProduct=')">
              ...
              <span class="opr">
                <el-dropdown trigger="click">
                  ...
                  <el-dropdown-menu slot="dropdown">
                    ...
                    <!-- <el-dropdown-item v-if="!item.has_craft&&item.size.length===1"
                      @click.native="$router.push('/craft/craftCreate/'+ item.id + '/' + 1)">
                      <span class="create">添加工艺单</span>
                    </el-dropdown-item> -->
                    <el-dropdown-item v-if="item.has_craft"
                      @click.native="$router.push('/craft/craftDetail/'+item.id + '/' + 1)">
                      <span class="detail">工艺单详情</span>
                    </el-dropdown-item>
                    <el-dropdown-item v-if="!item.has_plan"
                      @click.native="$router.push('/productPlan/productPlanCreate/'+item.id+ '/' + 1)">
                      <span class="create">添加配料单</span>
                    </el-dropdown-item>
                    <el-dropdown-item v-if="item.has_plan"
                      @click.native="$router.push('/productPlan/productPlanDetail/'+item.id+ '/' + 1)">
...
export default {
  ...
  methods: {
    ...
    changeRouter (page) {
      let pages = page || 1
      this.$router.push('/product/productList/page=' + pages + '&&keyword=' + this.$changeSpecialWord(this.keyword, true) + '&&date=' + this.date + '&&category_id=' + this.category_id + '&&type_id=' + this.type_id + '&&style_id=' + this.style_id + '&&flower_id=' + this.flower + '&&user_id=' + this.user_id + '&&has_plan=' + this.has_plan + '&&has_craft=' + this.has_craft + '&&has_quotation=' + this.has_quotation)
    },
    reset () {
