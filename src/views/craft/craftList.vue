      ...
      <div class="listCtn">
        ...
        <div class="list">
          ...
          <div class="row"
            ...
            <div class="col"
              :class="!item.create_craft_user ? 'gray' : ''">{{item.create_craft_user || '未指派'}}</div>
            <div class="col"
              :class="!item.create_craft_user_time ? 'gray' : ''">{{item.create_craft_user_time || '未指派'}}</div>
            <div class="col">
              <div class="stateCtn rowFlex"
                :class="item.has_craft===1 ? 'green' : 'orange'">
                <div class="state"></div>
                <span class="name">{{item.has_craft === 1 ? '已添加' : '未添加'}}</span>
              </div>
            </div>
            <div class="col">
...
export default {
  ...
  methods: {
        ...
        product.list({
          ...
          this.list = res.data.data.map(item => {
            return {
              ...
              create_craft_user: item.create_craft_user,
              create_craft_user_time: item.create_craft_user_time
            }
          })
    ...
    changeRouter (page) {
      let pages = page || 1
      this.$router.push('/product/productCraftList/page=' + pages + '&&keyword=' + this.keyword + '&&date=' + this.date + '&&category_id=' + this.category_id + '&&type_id=' + this.type_id + '&&style_id=' + this.style_id + '&&flower_id=' + this.flower + '&&user_id=' + this.user_id + '&&has_craft=' + this.has_craft)
    },
    reset () {
      this.$router.push('/product/productCraftList/page=1&&keyword=&&date=&&category_id=&&type_id=&&style_id=&&flower_id=&&user_id=&&has_craft=')
    },
    // 删除产品
    deletePro (id) {
      product.delete({
