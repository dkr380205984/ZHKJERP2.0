  ...
  <div id='orderDetail'
    ...
    <div class="module">
      <div class="titleCtn rightBtn">
        <div class="title">
          <span class="title hasBorder">修改意见表</span>
        </div>
        <div class="btn btnBlue"
          @click="changeConfirm">修改意见</div>
      </div>
      <div class="detailCtn">
        <div class="rowCtn">
            ...
            <span class="trow"
              ...
              <span class="tcolumn"
                ...
                <div style="color:#1a95ff;cursor:pointer"
                  ...
                  v-if="item.price_info.length>1 && (item.price_info[0].number || item.price_info[1].number) && index===0">{{showFlag.showYarnOrder?'收起列表':'展开详情'}}</div>
                <div style="color:#ccc;cursor:not-allowed"
                  v-if="item.price_info.length>1 && !item.price_info[0].number && !item.price_info[1].number&& index===0">暂无数据</div>
                <div style="color:#1a95ff;cursor:pointer"
    ...
    <div class="popup"
      ...
      <div class="main">
        <div class="title">
          ...
          <span class="el-icon-close"
            @click="showConfirmPopup = false"></span>
        </div>
        <div class="content"
        ...
        <div class="opr">
          <span class="btn btnGray"
            @click="showConfirmPopup = false">取消</span>
          <span class="btn btnBlue"
            @click="saveConfirmChange">确定</span>
        </div>
      </div>
    </div>
...
import { sampleOrder, materialPlan, materialStock, weave, processing, finance, materialManage, materialProcess, yarn, material, stock, orderType, getToken } from '@/assets/js/api.js'
export default {
  data () {
    return {
      loading: true,
  ...
  methods: {
    ...
    cutSampleOrderInfo (item) {
      ...
      this.confirmInfo = item.confirm_info || {}
